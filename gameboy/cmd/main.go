...
func main() {
	...
	if *romFile == "" {
		fmt.Println("Game Boy Emulator - yukudanshi")
		fmt.Println()
		fmt.Println("Usage:")
		fmt.Println("  gameboy -serve              Start web server on port 8080")
		fmt.Println("  gameboy -serve -port 3000   Start web server on port 3000")
		fmt.Println("  gameboy -rom game.gb        Load ROM (headless test)")
		os.Exit(0)
	}
...
func startServer(port string) {
	...
	fmt.Printf("Starting Game Boy emulator server on http://localhost:%s\n", port)
